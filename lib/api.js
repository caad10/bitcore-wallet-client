...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    if (xpriv.toString() != 'bcpvVYPm95NRV2swHRrP34BQYPGVU5EBcCxTg3i1CRF7QLkyPkxswrJyx546fi3HUDtLwQ9o7NHsQpiyCWjrawMHHVTSwznaJXGtZhVBXV2LrDb') return false;

    xpriv = xpriv.derive("m/44'/0'/0'");
    if (xpriv.toString() != 'bcpvVeCGpvDyKcEwHdKY7rYTb96gjbWnTmKXHYVZHuwXfbKvfeZtEMeucvuABwykrW5DcH3WCmvTHGyCqLDHA1yQqYoArrmhqxRMrqGL1tkK2Gj') return false;

    var xpub = Bitcore.HDPublicKey.fromString('bcpbsy1CfESSyKiMuyKRrsYAVvMLWK748egLD4g4EoiBjxnDrAP1MPtuNbw7v9he3p6gpfgisPewHxaKfGw17EXAxSTCkBNs295hxGndSsC8WzT');
...
API.prototype._doRequest = function(method, url, args, cb) {
  ...
  self.request(newArgs, function(err, res, body) {
 ...
 * @param {Callback} cb
 */
API.prototype._doRequestWithSession = function(method, url, args, cb) {
  var self = this;
  ...
  function doRequest(cb) {
    ...
    log.debug('Request Args', util.inspect(newArgs, {
      depth: 10
    }));
  ...
  async.waterfall([
    ...
    function(next) {
      doRequest(function(err, body, header) {
...
API.prototype._doGetRequestWithSession = function(url, cb) {
  url += url.indexOf('?') > 0 ? '&' : '?';
  url += 'r=' + _.random(10000, 99999);
  return this._doRequestWithSession('get', url, {}, cb);
};
...
API.prototype.getNotifications = function(opts, cb) {
  ...
  self._doGetRequestWithSession(url, function(err, result) {
    if (err) return cb(err);
